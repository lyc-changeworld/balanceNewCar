///////////////////////////////////////////////////////////////////////////////
//
// IAR ANSI C/C++ Compiler V7.20.2.7424/W32 for ARM       15/Aug/2016  15:07:14
// Copyright 1999-2014 IAR Systems AB.
//
//    Cpu mode     =  thumb
//    Endian       =  little
//    Source file  =  E:\LPLD_OSKinetis_V3.1\lib\LPLD\HW\HW_LPTMR.c
//    Command line =  
//        E:\LPLD_OSKinetis_V3.1\lib\LPLD\HW\HW_LPTMR.c -D LPLD_K60 -D
//        USE_K60D10 -lCN
...
//        ARM_MATH_CM4
//    List file    =  
//        E:\LPLD_OSKinetis_V3.1\project\balanceNewCar\iar\FLASH\List\HW_LPTMR.s
//
///////////////////////////////////////////////////////////////////////////////
...
// E:\LPLD_OSKinetis_V3.1\lib\LPLD\HW\HW_LPTMR.c
//    1 /**
//    2  * @file HW_LPTMR.c
...
LPLD_LPTMR_GetPulseAcc:
        MOVS     R0,#+1
        LDR.N    R1,??DataTable7_10  ;; 0x4004000c
        STR      R0,[R1, #+0]
//  188    return (uint16)LPTMR0->CNR;
...
//  190 
//  191 /*
//  192  *  LPLD_LPTMR_DelayMs
//  193  *  低功耗定时器延时n个ms
//  194  * 
//  195  * 参数:
//  196  *    period_ms--设定定时器的定时值 单位ms
//  197  *    最大65535ms
//  198  *
//  199  *  输出:
//  200  *      无
//  201  */
        ...
        SECTION `.text`:CODE:NOROOT(1)
        THUMB
//  202 void LPLD_LPTMR_DelayMs(uint16 period_ms)
//  203 {
//  204   if(!period_ms)
LPLD_LPTMR_DelayMs:
        UXTH     R0,R0            ;; ZeroExt  R0,R0,#+16,#+16
...
//  206   
//  207   /* 开启LPTMR时钟 */
//  208   SIM->SCGC5|=SIM_SCGC5_LPTIMER_MASK; 
...
//  209   
//  210   /* 设置毫秒延时计数 */
//  211   LPTMR0->CMR = period_ms; 
        UXTH     R0,R0            ;; ZeroExt  R0,R0,#+16,#+16
        LDR.N    R1,??DataTable7_6  ;; 0x40040008
        STR      R0,[R1, #+0]
//  212   
//  213   /* 设置LPTMR不分频采用1kHz LPO为时钟源 */
//  214   LPTMR0->PSR = 0|LPTMR_PSR_PCS(1)|LPTMR_PSR_PBYP_MASK; 
        MOVS     R0,#+5
        LDR.N    R1,??DataTable7_5  ;; 0x40040004
        STR      R0,[R1, #+0]
//  215   
//  216   /* 开启定时器 */
//  217   LPTMR0->CSR |= LPTMR_CSR_TEN_MASK; 
        LDR.N    R0,??DataTable7_4  ;; 0x40040000
        LDR      R0,[R0, #+0]
        ORRS     R0,R0,#0x1
        LDR.N    R1,??DataTable7_4  ;; 0x40040000
...
//  218 
//  219   /* 等待计数达到设定值 */
//  220   while (!(LPTMR0->CSR & LPTMR_CSR_TCF_MASK));
...
//  221   
//  222   /*清除定时比较标志 */
//  223   LPTMR0->CSR &= ~LPTMR_CSR_TEN_MASK;
        LDR.N    R0,??DataTable7_4  ;; 0x40040000
        LDR      R0,[R0, #+0]
        LSRS     R0,R0,#+1
        LSLS     R0,R0,#+1
...
//  228 /*
//  229  * LPTMR中断处理函数
//  230  * 与启动文件startup_K60.s中的中断向量表关联
//  231  * 用户无需修改，程序自动进入对应通道中断函数
//  232  */

        SECTION `.text`:CODE:NOROOT(1)
        THUMB
//  233 void LPT_IRQHandler(void)
//  234 {
LPT_IRQHandler:
        PUSH     {R7,LR}
//  235   LPTMR0->CSR|=LPTMR_CSR_TCF_MASK;  //清除LPT比较标志
        LDR.N    R0,??DataTable7_4  ;; 0x40040000
        LDR      R0,[R0, #+0]
        ORRS     R0,R0,#0x80
        LDR.N    R1,??DataTable7_4  ;; 0x40040000
...
?_0:
        DATA
        DC8 "E:\\LPLD_OSKinetis_V3.1\\lib\\LPLD\\HW\\HW_LPTMR.c"
        DC8 0, 0
