...
FuzzyStruct Direct_P = { //模糊方向P
  ...
  0,
  0,
  { -60, -30, -15, 0, 15, 30, 60 }, //{ -70, -30, -15, 0, 15, 30, 70 },
  { -10, -8, -5,  0,  5, 8, 10 },// { -10, -8, -5,  0,  5, 8, 10 },
  //  {  10, 11, 11, 12, 12,13, 13  },
    {  600, 660, 680, 735, 790,850, 925  },
...
void DirectionSpeedControl(void)//10ms
{
  ...
  avg_Speed=speed_avg*speed_avg/1400;
  if(avg_Speed - 3 < 0)
    avg_Speed = 3;
...
//  f_gyro_D=gyro_speed_x*gyro_D;//陀螺仪限制打角过冲
  ...
  if(fabs(f_gyro_D)>fabs(AngleControlOut+DirectSpeedControlOut1)*0.5)
  {
    if(f_gyro_D>0)
      f_gyro_D = fabs(AngleControlOut+DirectSpeedControlOut1)*0.5;
    else
      f_gyro_D = - fabs(AngleControlOut+DirectSpeedControlOut1)*0.5;
  }
...
//  else if(R_PowerValue<0)   R_PowerValue-=RightMotor_Dead;
  ...
  if(L_PowerValue<0 && R_PowerValue > 1000)
    L_PowerValue = 0;
  if(R_PowerValue<0 && L_PowerValue > 1000)
    R_PowerValue = 0;
  
  //左电机限幅
  if(L_PowerValue< -ADJUST_POWER)   L_PowerValue= -ADJUST_POWER;
  //if(L_PowerValue< 600)   L_PowerValue= 600;
  if(L_PowerValue> ADJUST_POWER)    L_PowerValue= ADJUST_POWER;
  //右电机限幅               
  if(R_PowerValue< -ADJUST_POWER)   R_PowerValue= -ADJUST_POWER;
  //if(R_PowerValue< 600)   R_PowerValue= 600;
  if(R_PowerValue> ADJUST_POWER)    R_PowerValue= ADJUST_POWER;
  //左电机输出 
