...
#include "oled.h"
#include "HW_GPIO.H"
GPIO_InitTypeDef gpio_init_struct0;
...
void init_gpio_oled()
{
  ...
  gpio_init_struct0.GPIO_Pins = GPIO_Pin15|GPIO_Pin16|GPIO_Pin13|GPIO_Pin12;
  gpio_init_struct0.GPIO_Dir = DIR_OUTPUT;
  gpio_init_struct0.GPIO_Output = OUTPUT_L;
  gpio_init_struct0.GPIO_PinControl = IRQC_DIS;
  LPLD_GPIO_Init(gpio_init_struct0);
}
/********************数字转字符显示*************************/
void  Num2Str(uint8 *str,int32 number)
{
  static uint8 i,j,Num_Wide;
  static int32 number_sign;
  static int32 temp;
  j=0;
  if(number<0)
  {
  ...
  number_sign = temp = number;
  if(number>=10)
  {
    for(Num_Wide = j;temp != 0;Num_Wide++)
    {
      temp = temp/10;
    }
    for(i=0;i<Num_Wide && number_sign!=0;i++)
    {
      str[Num_Wide-i-1] = table[number_sign%10];
...
void LCD_P6x8s(byte x,byte y,int32 number)
{
  byte c=0,i=0,j=0;
  uint8 string[10];
...
//输出汉字和字符混合字符串
void LCD_Print(byte x, byte y, uint8 ch[])
{
  uint8 ch2[3];
  uint8 ii=0;        
  while(ch[ii] != '\0')
...
void Show_Number(uint8 X,uint8 Y,int32 number)
{
  ...
  if(number/1000 > 0)
  {
    LCD_P6x8char(X+6,Y, number%10000/1000+48);
    LCD_P6x8char(X+12,Y,number%1000/100+48);
    LCD_P6x8char(X+18,Y,number%100/10+48);
    LCD_P6x8char(X+24,Y,number%10+48); 
  }
