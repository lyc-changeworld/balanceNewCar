...
/*********角度控制系数量**********/           
float angle_P=1000,angle_D=5/16.0;//2000 10  5/16.0
...
{ 
  ...
  nP=-angle_P*(angle-Angle_Zero+4+SA);//angle-MMA8451_Y+4   +3
...
void DirectionSpeedControl(void)//10ms
{
  ...
  //nSpeed=(speed_L+speed_R)*SPEED_VALUE/2.0;//车轮当前的转速*SPEED_VALUE 
  //nSpeed = fabs(nSpeed);
//  if(Speed_MAX<=140)
//  {
//    avg_Speed=speed_avg/13;
//  }else if(Speed_MAX<=160)
//  {
//    avg_Speed=speed_avg/14;
//  }else if(Speed_MAX<=180)
//  {
//    avg_Speed=speed_avg/15;
//  }else
//  {
//    avg_Speed=speed_avg/16;
//  }
  avg_Speed=speed_avg/13;//    /30.0
  if(avg_Speed - 1 < 0)
    avg_Speed = 1;
  ...
  //  SpeedAlone_P1 = Fuzzy_Update(&Direct_P,New_dir_err*100,D_err*100);
    ...
    // SpeedAlone_D1=0;
    SpeedAlone_D1 = SpeedAlone_P1+3000;
...
//  speedout2= -speedout1;
  ...
  //   speedout1=-speed_L+ nSpeed*(1+offset);//左右轮预期速度值 按当前速度值计算出
  //  speedout2=-speed_R + nSpeed*(1-offset);
  //  speedout1=+offset;//-speed_L_real+speed_avg_real+
  //  speedout2=-offset;//-speed_R_real+speed_avg_real
  //------方向控制量拟合到差速控制上 两轮差速控制PID----------//
  Speederror12=Speederror11;
  Speederror11=speedout1;
  
//  Speederror22=Speederror21;
//  Speederror21=speedout2;
  
  SpeedError1=Speederror11-Speederror12;
//  SpeedError2=Speederror21-Speederror22;
  
  //  avg_Power = fabs(L_PowerValue+R_PowerValue)/2;
  fP1=speedout1*SpeedAlone_P1;  //比例运算 
//  fP2=speedout2*SpeedAlone_P2;
...
//  fD2=SpeedError2*SpeedAlone_D2;
  
  
//  fD1=D_err*SpeedAlone_D1; //微分运算
//  fD2=D_err*SpeedAlone_D2;
  
//  speed_control_old1 = speed_control_new1;
//  speed_control_old2 = speed_control_new2;

  speed_control_new1 = fP1+fD1;
  speed_control_new2 = -speed_control_new1;
  //speed_control_new2 = fP2+fD2;
}
//差速度输出量
