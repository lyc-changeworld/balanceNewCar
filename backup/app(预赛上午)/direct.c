...
void cal_offset(void)
{
  ...
  if(nowADBase>254)
    nowADBase = 254;
 // nowADBase = AD1Basic*pow(1+sin((12.5-angle)/60),2);
  first_offset[0] = (AD_result[2] - AD_result[0])/(1+AD_result[0] + AD_result[2]);
...
//  
  ...
  if(AD_result[1] > AD1Basic*0.7  )
  {
    if(first_offset[0]>0)
    {
      offsetTemp = first_offset[0]*0.6 + first_offset[1]*0.4;
    }else
    {
      offsetTemp = first_offset[0]*0.6 - first_offset[1]*0.4;
    }
  ...
  }else
  {
    ...
    }else if(first_offset[0]<0)
    {
      offsetTemp = -first_offset[1];
    }
    //offsetTemp = his_offset[0];
  }

  if(AD_result[1]>AD_result[0] && AD_result[1]>AD_result[2])
  {
    offsetTemp = first_offset[0];
  }
//  
//  if(AD_result[1]>40 &&fabs(first_offset[1])>0.35 && fabs(first_offset[0])< 0.2 && straightTime > 200)
//  {
//    offsetTemp = first_offset[0];
//  }
//  
